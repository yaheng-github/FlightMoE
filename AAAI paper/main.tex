...
Table~ef{tab:phase_test_open} reports per-phase metrics on the open test set. Performance is consistently high across phases, with a slight drop during hover and decel. Table~ef{tab:fault_test_open} breaks down open-set performance by unseen fault type. The model generalizes well to accelerometer, gyroscope, magnetometer, and GPS faults, while barometer anomalies are relatively harder.
